...
class SongRanker {
    constructor(initialSongs) {
        ...
        this.songs = initialSongs.map(s => (Object.assign({}, s)));
    }
    initialize() {
    ...
    addSong(song) {
        ...
        const newSong = {
            ...
            releaseYear: (_a = song.releaseYear) !== null && _a !== void 0 ? _a : 0
        };
        ...
        rl.on('line', (line) => {
            ...
            switch (cmd) {
                ...
                case 'add': {
                    ...
                    if (parts.length < 2) {
                        console.log('Use: add title|album|year(optional)');
                        break;
                    }
                    const title = parts[0];
                    const album = parts[1];
                    const year = parts[2] ? parseInt(parts[2], 10) : undefined;
                    this.addSong({ title, album, artist: 'Taylor Swift', releaseYear: year !== null && year !== void 0 ? year : 0 });
                    break;
                }
                case 'save': {
                    const p = parts[1];
                ...
                case 'load': {
                    const p = parts[1];
                    if (!p) {
                        console.log('Provide a path to load from');
                        break;
                    }
                    this.loadFromFile(p);
                    break;
...
exports.SongRanker = SongRanker;
exports.default = SongRanker;
